                            ...
                            @if ($role->habilitated)
                                <td class="py-2 px-4">
                                    <form action="{{ route('role.destroy', $role->idRole) }}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                            class="bg-red-500 text-white py-1 px-3 rounded hover:bg-red-600">Borrar</button>
                                    </form>
                                </td>
                            @else
                                <td class="py-2 px-4">
                                    <form action="{{ route('role.alta', $role->idRole) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="bg-green-500 text-white py-1 px-3 rounded hover:bg-green-600">Habilitar</button>
                                    </form>
                                </td>
                            @endif
                        </tr>
